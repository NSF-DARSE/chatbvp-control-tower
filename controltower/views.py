...
def event_detail(request, event_id: int):
    ...
    # If last run succeeded, mark DONE; if failed, mark FAILED (showing retry button in uI)
    if latest_run:
...
def run_agent_from_ui(request, event_id: int):
    ...
    # Running  the agent by calling the same endpoint internally
    run_event_agent(request, event_id)

    # After run completes (status updated in DB and), redirect to detail page
